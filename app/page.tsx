        ...
        <main className={`${dark ? "bg-black text-white" : "bg-white text-black"} transition-colors duration-700 min-h-screen font-sans selection:bg-cyan-500/30 overflow-x-hidden relative flex flex-col items-center`}>
            ...
            <div className="w-full max-w-5xl px-6 space-y-32 md:space-y-60 py-20 md:py-40 relative z-20">
                ...
                <section id="experience" className="scroll-mt-32">
                    <h2 className="text-[10px] tracking-[1em] uppercase opacity-30 mb-16 text-center font-bold">Experience</h2>
                    <div className="grid md:grid-cols-2 gap-8">
                        {[
                            ...
                            <motion.div key={idx} whileHover={{ y: -8 }} className={`p-8 md:p-10 rounded-[2.5rem] border ${dark ? "bg-neutral-900 border-white/10" : "bg-neutral-50 border-black/5"} shadow-xl transition-all`}>
                                <div className="flex flex-col items-start gap-6">
                                    <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center p-3 bg-white overflow-hidden shadow-md">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-cyan-500 font-bold text-sm mt-1 uppercase tracking-widest">{exp.company}</p>
                                    </div>
                                    <div className="space-y-4 pt-6 border-t w-full border-white/10 text-sm md:text-base opacity-60 leading-relaxed">
                                        <p className="text-[10px] font-black tracking-widest uppercase opacity-40">{exp.period}</p>
                        ...
                        {skills.map((skill) => (
                            <Magnetic key={skill.name} distance={0.3}>
                                <div className="relative group">
                                    <motion.div whileHover={{ scale: 1.1 }} className={`w-20 h-20 md:w-24 md:h-24 rounded-full border flex items-center justify-center p-5 transition-all duration-500 ${dark ? "border-white/10 bg-neutral-900/50 hover:border-cyan-500/50" : "border-black/10 bg-neutral-50 hover:border-cyan-500/50"}`}>
                                        <img src={skill.icon} alt={skill.name} className="w-10 h-10 md:w-12 md:h-12 object-contain grayscale group-hover:grayscale-0 transition-all" />
                                    </motion.div>
                                    <div className="absolute -top-16 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none z-50">
                                        <div className="bg-cyan-600 text-white px-4 py-2 rounded-xl shadow-2xl flex flex-col items-center min-w-[110px] relative">
                                            <span className="text-[10px] font-black uppercase tracking-widest mb-1">{skill.name}</span>
                                            <span className="text-[8px] font-bold opacity-80 uppercase tracking-tighter">{skill.level}</span>
                ...
                <section id="projects" className="scroll-mt-32">
                    <h2 className="text-[10px] tracking-[1em] uppercase opacity-30 mb-16 text-center font-bold">Featured Projects</h2>
                    <div className="space-y-12 md:space-y-24">
                        {[
                            ...
                            <motion.div key={idx} className={`p-10 md:p-16 rounded-[3rem] border ${dark ? "bg-neutral-900 border-white/10" : "bg-neutral-100 border-black/10"} group hover:border-cyan-500/30 transition-colors`}>
                                <h3 className="text-3xl md:text-4xl font-bold mb-6 italic">{proj.title}</h3>
                                <p className="opacity-60 max-w-2xl text-lg md:text-xl mb-10">{proj.desc}</p>
                                <div className="flex flex-wrap gap-5 text-xs font-bold uppercase tracking-widest opacity-30 group-hover:opacity-100 transition-opacity">
                ...
                <section id="contact" className="scroll-mt-32">
                    <h2 className="text-4xl md:text-6xl font-extrabold mb-12 leading-tight">I&apos;ve got just what you need. <br /><span className="underline decoration-cyan-500 underline-offset-8">Let&apos;s talk.</span></h2>
                        ...
                        <div className="space-y-8 md:space-y-12">
                            {[
                                ...
                                <div key={idx} className="flex items-center gap-6 group cursor-default">
                                    ...
                                    <span className="text-lg md:text-xl opacity-60 group-hover:opacity-100 transition-opacity font-medium">{c.val}</span>
                                </div>
                            ))}
                        </div>
                        <form className="space-y-5">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <input type="text" placeholder="Name" className={`p-5 rounded-2xl border ${dark ? "bg-neutral-900 border-white/10 text-white" : "bg-neutral-50 border-black/10 text-black"} outline-none focus:border-cyan-500 transition-all text-sm w-full`} />
                                <input type="email" placeholder="Email" className={`p-5 rounded-2xl border ${dark ? "bg-neutral-900 border-white/10 text-white" : "bg-neutral-50 border-black/10 text-black"} outline-none focus:border-cyan-500 transition-all text-sm w-full`} />
                            </div>
                            <textarea placeholder="Message" rows={5} className={`w-full p-5 rounded-2xl border ${dark ? "bg-neutral-900 border-white/10 text-white" : "bg-neutral-50 border-black/10 text-black"} outline-none focus:border-cyan-500 transition-all text-sm resize-none`} />
                            <button type="button" className="w-full py-5 rounded-2xl bg-cyan-600 text-white font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-4 hover:bg-cyan-500 transition-all active:scale-[0.98]">Send Message <Icons.Send /></button>
                        </form>
                    </div>
                </section>
            </div>

            <footer className="w-full py-16 md:py-24 text-center border-t border-white/5 mt-20">
                <p className="opacity-20 text-[10px] md:text-xs tracking-[1em] uppercase font-bold px-4">Omkar Sinare • Pune, Maharashtra</p>
            </footer>
        </main>
    );
}
